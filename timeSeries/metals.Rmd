---
title: 'METALS'
author: "Tajudeen Abdulazeez, Group 0"
output: 
...
## {.sidebar}
###
<img src="https://styleguide.ischool.syr.edu/assets/logos/block/iSchoolBlock-OutlineswithSyracuseUniversity_Orange.png" alt="Syracuse University" width="150" height="150">
...
```{r}
renderPlot({
  pacf(coredata(data.xts[,1:3])) # returns
})
...
```{r}
renderPlot({
  pacf(coredata(data.xts[,4:6])) # sizes
})
```
...
alpha.tolerance <- .95
#alpha.tolerance <- reactive({ifelse(input$alpha.e>1,0.99,ifelse(input$alpha.e<0,0.001,input$alpha.e))})
...
In practice, however, the risk-free rate does not exist because even the safest investments carry a very small amount of risk. Thus, the interest rate on a three-month U.S. Treasury bill is often used as the risk-free rate for U.S.-based investors. (https://www.investopedia.com/terms/r/risk-freerate.asp).

**Calculation of Risk-free rate**

1. Most of the time the calculation of the risk-free rate of return depends on the time period that is under evaluation. If the time period is for one year or less than one year than one should go for the most comparable government security i.e., Treasury Bills. For example, if the treasury bill quote is .389 then the risk-free rate is .39%.

1. If the time duration is in between one year to 10 years than one should look for Treasury Note. For Example: If the Treasury note quote is .704 than the calculation of risk-free rate will be 0.7%

1. If the time period is more than one year than one should go for Treasury Bond For example if the current quote is 7.09 than the calculation of risk-free rate of return would be 7.09%.
...
\large
Risk Free Rate = (\frac{(1 + Goverrnment Bond)}{(1 + inflation Rate)}) -1

$$


- Risk free rate obtained from treasury bill
- and using 52 weeks number of trading days
...
tb <- 1.83/253 ## 52 weeks daily quoate of bank dicount 
inflationRate <- 1.81/253 ## 2018 t0 2019 from http://www.in2013dollars.com/us/inflation/2018?amount=100
  
#mu.free <- .00011 ## input value of daily risk-free interest rate
mu.free <- ((1 + tb)/(1 + inflationRate))-1
mu.free
```
...
- Kevin Cerullo for the freight example worked in detail through this analytics maze including the key insights.

- Stefan Hoeller for working out portfolio examples of bodies versus tails of returns distributions.

- Andrew Foote for general analytical support and advice.

- Any errors and omissions are the sole responsibility of William Foote. This application is for educational purposes only.
...
* $250 million denominated in euros 
* $195 million in nickel
* $25 million in copper
* $30 million in aluminium


With constraints - No long

* $250 million denominated in euros 
* $22.5 million in nickel
* $112.5 million in copper
* $115 million in aluminium


With constraints - No short
...
- The working capital account of \$250 million euro should be allocated as follows: buy \$195 million in nickel, \$25 million in copper, \$30 million in aluminium.

- There is a strong correlation (0.89) between Ni (Nickel) and Cu (copper) due to shared applications.
...
The capital assets should be adjusted and distributed as:  \$195 million in nickel, \$25 million in copper, \$30 million in aluminium.
This will make more sense because Ni and Cu and strongly positively correlated. 
Based on the historical records, Ni is more statble and putting large sum of the capital in this metal might be less risky and can be considered as a calculated risk with promising returns.
...
* https://www.eia.gov/<energy>/data.php	
* https://fred.stlouisfed.org/categories
* https://finance.yahoo.com/quote/<sym>/history?ltr=1 
